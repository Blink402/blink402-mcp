...
exports.buildUsdcPaymentTransaction = buildUsdcPaymentTransaction;
exports.validateWalletAddress = validateWalletAddress;
exports.getUsdcMintForNetwork = getUsdcMintForNetwork;
exports.usdcToAtomic = usdcToAtomic;
exports.atomicToUsdc = atomicToUsdc;
const web3_js_1 = require("@solana/web3.js");
const spl_token_1 = require("@solana/spl-token");
const config_1 = require("@blink402/config");
const logger = (0, config_1.createLogger)('@blink402/solana-transaction-builder');
/**
 ...
 */
const PAYAI_FEE_PAYER = new web3_js_1.PublicKey("2wKupLR9q6wXYppw8Gr2NvWxKBUqm4PPJKkQfoxHDBg4");
/**
 * USDC Mint addresses (network-dependent)
 */
const USDC_MINT_MAINNET = new web3_js_1.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
const USDC_MINT_DEVNET = new web3_js_1.PublicKey("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");
 ...
 * ```
 */
async function buildUsdcPaymentTransaction(params) {
    ...
    const amountAtomic = BigInt(Math.round(amountUsdc * 1000000));
    // Get Associated Token Addresses for payer and merchant
    let payerATA;
    let merchantATA;
    try {
        logger.info('Getting payer ATA...', { payer: payer.toBase58() });
        payerATA = await (0, spl_token_1.getAssociatedTokenAddress)(USDC_MINT, payer);
    ...
    try {
        ...
        merchantATA = await (0, spl_token_1.getAssociatedTokenAddress)(USDC_MINT, merchant);
    ...
    try {
        const payerAccount = await (0, spl_token_1.getAccount)(connection, payerATA);
        const balance = Number(payerAccount.amount) / 1000000;
    ...
    if (!merchantAtaInfo) {
        throw new Error(`Merchant USDC account doesn't exist. Please contact the merchant to set up their USDC account.`);
    }
    ...
    const instructions = [
        ...
        web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({ units: 40000 }),
        ...
        (0, spl_token_1.createTransferCheckedInstruction)(payerATA, USDC_MINT, merchantATA, payer, amountAtomic, USDC_DECIMALS)
    ];
 ...
 */
function validateWalletAddress(address, type = 'wallet') {
    try {
 ...
 */
function getUsdcMintForNetwork(network) {
 ...
 */
function usdcToAtomic(amountUsdc) {
    return BigInt(Math.round(amountUsdc * 1000000));
}
/**
 ...
 */
function atomicToUsdc(amountAtomic) {
    return Number(amountAtomic) / 1000000;
}
