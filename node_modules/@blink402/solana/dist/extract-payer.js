...
exports.extractPayerFromTransaction = extractPayerFromTransaction;
exports.extractPayerWithRetry = extractPayerWithRetry;
const config_1 = require("@blink402/config");
const logger = (0, config_1.createLogger)('@blink402/solana-payer-extraction');
/**
 ...
 */
async function extractPayerFromTransaction(connection, signature) {
    try {
        ...
        const feePayer = transaction.transaction.message.accountKeys.find(account => account.signer);
        if (feePayer) {
 ...
 */
async function extractPayerWithRetry(connection, signature, maxRetries = 3) {
    let lastError = null;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            logger.warn('Retrying payer extraction after error', { attempt: attempt + 1, maxRetries, error: lastError.message });
